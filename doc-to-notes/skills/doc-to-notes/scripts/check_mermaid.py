...
Obsidian HTML/Markdown 切换规则：
    [E5] </div> 后紧接 > 引用块/callout 但缺少空行
         根本原因：Obsidian 渲染器在处理 HTML 块时，必须有一个空行才能切换回 Markdown 解析模式。
         没有空行时，> 符号被当作 HTML 上下文的一部分，引用块/callout 以纯文本直接展示。
         修复：在 </div> 和 > 引用块之间插入一个空行即可。
"""
...
# ── 检查 HTML 块与 Markdown 切换 ─────────────────────────────────────────────

def check_html_div_quote_gap(md: str, filepath: str, fenced_lines: set) -> list:
    """检测 </div> 后紧接 > 引用块但缺少空行 [E5]。

    Obsidian 渲染器处理 HTML 块时必须有一个空行才能切换回 Markdown 解析模式。
    缺少空行时，> 被当作 HTML 上下文，callout / 引用块以纯文本展示。
    """
    issues = []
    rel = os.path.basename(filepath)
    lines = md.splitlines()
    ...
    for i in range(len(lines) - 1):
        lineno = i + 1  # 1-based，当前行（</div> 行）
        if lineno in fenced_lines:
            continue
        if not lines[i].rstrip().endswith("</div>"):
            continue
        next_line = lines[i + 1]
        if next_line.strip().startswith(">"):
            next_lineno = i + 2
            issues.append(("ERROR", "E5", next_lineno,
                           f"{rel}:{next_lineno} — </div> 后缺少空行直接跟 > 引用块"
                           f"（Obsidian 停留在 HTML 模式，callout/引用块显示为纯文本）"
                           f": {next_line.strip()[:60]}"))
    return issues
...
def check_file(filepath: str) -> list:
    ...
    for s, e in extract_fenced_ranges(md):
        for ln in range(s, e + 1):
            fenced_lines.add(ln + 1)  # convert to 1-based
    ...
    issues.extend(check_html_div_quote_gap(md, filepath, fenced_lines))
    return issues
...
def main() -> int:
    ...
    if errors:
        ...
        print("  [E5] 在 </div> 与 > 引用块/callout 之间插入一个空行，让渲染器切换回 Markdown 解析模式")
