...
- **🆕 文档类型判定（关键，决定后续可视化强度与核查阈值）**：极客时间专栏覆盖技术与非技术两大类，二者的提炼重点截然不同，必须先判定类型并记录，供 Step 3（可视化强度）与 Step 6（`--type` 阈值）使用。
  - **`technical`（代码/架构密集型）**：文中含代码、架构图、配置表、命令行、接口定义等。典型：Flink/Java/分布式/数据库/前端工程专栏。重点是**还原代码、图表、参数**。
  - **`conceptual`（散文/管理/软技能型）**：通篇说理叙述，价值在**论证链、并列枚举、类比、作者归纳的"N个要素"**，几乎无代码。典型：技术管理、职业发展、沟通/认知/方法论专栏。重点是**保散文细节与逻辑链**。
  - **判定信号**：原文 Slate 文本中出现代码片段/命令/配置项/接口名 → `technical`；通篇为"第一/第二…""所谓的 X 就是…""我们可以从 N 个方面…"式说理 → `conceptual`。拿不准时，按"原文是否存在可提炼的代码/架构图"二选一：有 → technical，无 → conceptual。
  - **两类的差异化处理**（其余流程完全一致）：

    | 维度 | technical | conceptual |
    |---|---|---|
    | 可视化强度 | 代码块/Mermaid/表格密集，鼓励多图 | 可视化**降为辅助**：总结 1 个 mindmap 足矣，正文按需 1 个框架图；**不为凑数硬造卡片** |
    | 第一优先级 | 代码、架构、参数零失真 | **散文保真**：并列枚举、桥接逻辑、类比、"N个要素"逐条展开 |
    | Step 6 阈值 | `--type technical`（散文 60%） | `--type conceptual`（散文 80%） |
- 从 HTML 结构中优先提取关键元数据：
  - **文章标题**（`<title>` 或 `<h1>`）
...
- **建立章节级覆盖清单**：解析完成后，列出正文中所有有效章节级标题（按上面"章节级判定"识别，已过滤"全部留言"），格式如下（后续 Step 2 写作前逐一打勾核对）：
  ...
  ...
  ```
- **🆕 关键内容元素登记（`conceptual` 类必做，`technical` 类亦建议）**：章节覆盖清单只保证"大节不漏"，但散文型文章的内容流失往往发生在**节内的细粒度元素**上（实测一篇笔记 10 项结构核查全过，仍丢了 4 处正文）。因此除章节清单外，再登记以下高价值元素，Step 2 写作时逐一核对、Step 6 用脚本机械复验：
  - **数据/统计**：原文所有具体数字及其语境（如"500 位管理者""12 个话题""第 27 篇""7 篇文章"）。
  - **"N个X"归纳结构**：作者明确归纳的"N 个目的/N 个因素/N 大要素"（如"沟通四大目的""四个稳定因素"）。**每一项都要在笔记中单独成节或单独成条展开**，不能只点名不展开。
  - **并列枚举列表**：原文以冒号/顿号引出的并列子项，尤其是 ≥5 项的长枚举（如"技术开发工作：使用电脑、学习语言、设计算法、开发功能、遵循规范"）。**必须以列表形式保留每一项**，不得压缩为一句概述。
  - **桥接逻辑句**：连接两个论点的因果推导句（如"正因为如此……""这就是……之所以……的原因""这一样绝非偶然"）。这类句子是论证链的关节，**不得省略**。
  - **关键类比**：作者解释抽象概念用的类比（如"建立沟通通道，就像两个技术模块通信要先建立连接"）。管理/软技能类文章的核心解释机制，必须保留。

  登记格式（示例）：
  ```
  关键内容元素清单：
  - [ ] 数据: 500 位技术管理者 / 12 个话题 / 第 27 篇
  - [ ] N个结构: 沟通四大目的（建立通道·同步信息·表达情感·输出影响）→ 须逐条展开
  - [ ] N个结构: 四个稳定因素（管理逻辑·沟通通道·工具流程·影响力）→ 须逐条展开
  - [ ] 长枚举: 四项工作各自的子项（技术开发/项目管理/和下级/和上级，每项 5 子项）
  - [ ] 桥接逻辑: 优秀技术人被提拔 → 价值观被颠覆 的因果链
  - [ ] 类比: 沟通通道 ≈ 技术模块间的"连接"
  ```
...
- **🆕 "N个要素"结构的归属与展开规则**：原文中作者明确归纳的"N 个 XXX"（"四大目的""四个稳定因素""三大要素"等），无论出现在原文开头、中间还是收束段，一律归入 `## 原理解析`，并把**每个子项拆为独立 `###` 子节或独立列表项逐条展开**（每条至少一句解释"是什么/为什么"）。❌ 禁止只罗列名称不展开；❌ 禁止把这类结构塞进 `## 最佳实践`——最佳实践讲"怎么做"，原理解析讲"是什么/为什么"。`conceptual` 类文章中，此节通常是全文最重要的节。
- **🆕 并列枚举列表的保留规则**：原文中出现的并列子项（即便以行内冒号/顿号而非正式列表呈现，如"技术开发工作：使用电脑、学习语言、设计算法、开发功能、遵循规范"），必须在笔记中以 `- ` 列表或表格**完整保留每一个子项**，不得合并为一句描述性文字。尤其 ≥5 项的长枚举，Step 6 会用脚本逐项机械核验。
...
- **🚨 分段写入（必须严格遵守，禁止一次性写入全文）**：笔记正文通常超过 5000 字，若一次性生成全部内容，单次 LLM API 响应时间会超过 120 秒，触发 Cloudflare 代理层 524 超时错误。**必须按章节拆分为多步写入**，将每步生成量控制在约 800-1200 字以内。

  **分步写入流程**：

  **第 1 步 — 用 Write 工具创建文件**（仅含 Frontmatter + `## 关键概念` 章节）：
  使用 Write 工具写入文件，内容只到 `## 关键概念` 章节结尾的 `---` 分隔线为止（**不要写后续章节**）。

  **第 2 步起 — 每个后续 `##` 章节单独追加**：
  对每个后续章节，先将章节内容写入临时文件，再用 Bash 追加到主文件：
  ```bash
  # 将上一步 Write 工具写好的章节临时文件追加到主文件
  ...
  **推荐分段节点**（每步一个 Write + 一条 Bash cat >>）：
  | 步骤 | 内容 | 写入方式 |
  |---|---|---|
  | 1 | Frontmatter + `## 关键概念` | Write 工具（创建文件） |
  | 2 | `## 原理解析`（含所有 `###` 子节） | Write 临时文件 → cat >> 追加 |
  | 3 | `## 代码示例` | Write 临时文件 → cat >> 追加 |
  | 4 | `## 最佳实践 & 避坑指南` | Write 临时文件 → cat >> 追加 |
  | 5 | `## 作者金句` + `## [作者名]说`（若有） | Write 临时文件 → cat >> 追加 |
  | 6 | `## 思考题` | Write 临时文件 → cat >> 追加 |
  | 7 | `## 总结`（含 mindmap） | Write 临时文件 → cat >> 追加 |

  > **章节过长时**（如 `## 原理解析` 有 5 个以上 `###` 子节），可再拆为 2 步（每步写 2-3 个 `###` 子节），保证单步生成内容 ≤ 1200 字。
...
- [x] 思考题处理正确
- [x] 无外部资源污染
- [ ] 散文未过度压缩 ⚠️ verify_content.py RATIO=xx.x% < 阈值（type=conceptual/80%），FAIL
- [x] Mermaid 节点无 \n 换行
